...
Bool AudioFileCache::decodeFFmpeg(OpenAudioFile* file)
{
  auto on_frame = [](AVFrame *frame, int stream_idx, int stream_type, void *user_data) {
    ...
    const int frame_data_size = file->m_ffmpegFile->Get_Size_For_Samples(frame->nb_samples);
  ...
  file->m_ffmpegFile->Set_Frame_Callback(on_frame);
  file->m_ffmpegFile->Set_User_Data(file);

  // Read all packets inside the file
  while (file->m_ffmpegFile->Decode_Packet()) {
  }

  // Calculate the duration in MS
  file->m_duration = (file->m_totalSamples / (float)file->m_ffmpegFile->Get_Sample_Rate()) * 1000.0f;
...
void *AudioFileCache::openFile( AudioEventRTS *eventToOpenFrom )
{
  ...
  if (!openedAudioFile.m_ffmpegFile->Open(file)) {
	...
	if (eventToOpenFrom->isPositionalAudio()) {
		if (openedAudioFile.m_ffmpegFile->Get_Num_Channels() > 1) {
